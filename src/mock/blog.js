...
Mock.mock('/api/blogtype', 'get', {
  code: 0,
  msg: '',
  // 属性名为data，属性值为长度介于10-20之间的数组（包含边界）
  "data|10-20": [
    // data数组的每一项都是一个对象
    {
      // 属性名为id，属性值为自增的数字，自增的初始值为1
      "id|+1": 1,
      // 属性名为name，属性值为“分类<对象的id属性值>”
      name: "分类@id",
      // 属性名为articleCount，属性值为10-100的数字（包含边界），这里的“0”只用来确定类型
      "articleCount|10-100": 0,
      // 属性名为order，属性值为自增的数字，自增的初始值为1
      "order|+1": 1
    }
  ]
});
...
Mock.mock(/^\/api\/blog(\?.+)?$/, 'get', function(options){
  const limit = qs.parse(options.url).limit; // 从url中解析出页容量limit
  const data = {
    // 属性名为total，属性值为2000-3000的数字（包含边界），这里的“0”只用来确定类型
    'total|2000-3000': 0,
    // 属性名为rows，属性值为固定长度（limit || 10）的数组
    [`rows|${limit || 10}`]: [
      // rows数组的每一项都是一个对象
      {
        // 属性名为id，属性值为全局唯一的id
        id: '@guid',
        // 属性名为title，属性值为中文标题，长度为5-30个中文字符（包含边界）
        title: "@ctitle(5, 30)",
        // 属性名为description，属性值为5-15句话的中文段落（包含边界）
        description: "@cparagraph(5, 15)",
        category: {
          // 属性名为id，属性值为1-10的数字（包含边界），这里的“0”只用来确定类型
          "id|1-10": 0,
          // 属性名为name，属性值为“分类<对象的id属性值>”
          name: "分类@id"
        },
        // 属性名为scanNumber，属性值为0-3000的数字（包含边界），这里的“0”只用来确定类型
        "scanNumber|0-3000": 0,
        // 属性名为commentNumber，属性值为0-300的数字（包含边界），这里的“0”只用来确定类型
        "commentNumber|0-300": 0,
        // Mock.Random.image("300x250","#000","#fff","Random Image")返回随机的图片地址，尺寸为300x250，背景色为黑色，前景色为白色，图片上的文字为Random Image
        // 属性名为thumb，属性值从数组中随机选择一个元素，可能是图片地址，也可能是null
        // "thumb|1": [Mock.Random.image("300x250", "#000", "#fff", "Random Image"), null],
        // "thumb|1": ["@image(300x250, @color, #fff, @natural)", null],
        "thumb|1": [`@image(300x250, #008c8c, #fff, ${Mock.Random.natural(1, 9)})`, null],
        // 属性名为createDate，属性值为日期，格式为时间戳（字符串）
        createDate: "@date(T)"
      }
    ]
  };
...
<p>所以，CORS是一种更好的跨域解决方案。</p><h1 id="article-md-title-1">概述</h1><p><code>CORS</code>是基于<code>http1.1</code>的一种跨域解决方案，它的全称是<strong>C</strong>ross-<strong>O</strong>rigin <strong>R</strong>esource <strong>S</strong>haring，跨域资源共享。</p><p>它的总体思路是：<strong>如果浏览器要跨域访问服务器的资源，需要获得服务器的允许</strong></p><figure class="markdown-image">
      ...
      <img style="width:auto" src="https://mdrs.yuanjin.tech/img/image-20200421152122793.png" title="image-20200421152122793" alt="image-20200421152122793">
...
消息体中的数据</code></pre><p>当浏览器看到服务器允许自己访问后，高兴的像一个两百斤的孩子，于是，它就把响应顺利的交给js，以完成后续的操作</p><p>下图简述了整个交互过程</p><figure class="markdown-image">
      ...
      <img style="width:auto" src="https://mdrs.yuanjin.tech/img/image-20200421162846480.png" title="image-20200421162846480" alt="image-20200421162846480">
...
添加用户成功</code></pre><p>可以看出，当完成预检之后，后续的处理与简单请求相同</p><p>下图简述了整个交互过程</p><figure class="markdown-image">
      ...
      <img style="width:auto" src="https://mdrs.yuanjin.tech/img/image-20200421165913320.png" title="image-20200421165913320" alt="image-20200421165913320">
